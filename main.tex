...
\todo[inline]{in this thesis}I reproduce the model, analysing its behaviour under various variations from the original, quantifying its prediction accuracy working in conjunction with three different supervised algorithms used for classification purposes:
...
\end{itemize}
\todo[inline]{spiegare più a parole che a punti}
...
In general, the last 20 years have seen a growing number of fields in which a large quantity of digital data is available, the so called "big data" field. \todo[inline]{anche la potenza computazionale\ldots}
...
The input  samples consist in a grayscale image of the written digits, so to each pixel in the image is associated a value from 0 to 1 proportional to the saturation \todo[inline]{non è la saturazione, è l'intensità} of the image in that point.
...
\todo[inline]{spiegare meglio la sigmoide e la RELU?}
...
This is obtained thought gradient descent: \todo[inline]{spiegare meglio cosa sia il gradient descent}
...
\todo[inline]{spiegare anche che ci sono problemi di convergenza numerica}
...
The algorithm is said to be non-local, requiring information from all the above layers. \todo[inline]{far risaltare meglio il fatto che il termine locale o meno si riferisca ai layer}
...
\begin{itemize}
	...
	\item The sphere is defined for a Lebesgue norm with $p \geq 2$. \todo[inline]{spiegare meglio cosa la norma di Lebesgue}
\end{itemize}
...
Within regards to computational complexity, it was expected the computational complexity to be O(N) \todo[inline]{spiegare cosa sia la complessità computazionale}, where N is the number of epochs processed.
...
In about 20\% of cases the algorithm was observed converging to the pattern shown in figure \ref{nove}. \todo[inline]{fare riferimento a figure future è difficile da seguire\ldots}
...
\todo[inline]{spiegare il database MNIST}
...
By their own nature they are very sensible to outliers. \todo[inline]{giustificare! citazioni, esempi, etc\ldots}
